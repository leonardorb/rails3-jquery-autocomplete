    ...
    init: function(e) {
      ...
      jQuery(e).autocomplete({
        source: function( request, response ) {
          if(jQuery(e).attr('data-autocomplete').charAt(0) != "/") {
            var values = jQuery.parseJSON(jQuery(e).attr('data-autocomplete'));
            var term = (request.term).toUpperCase();
            var filteredValues = _.filter(values, function(el){
              return el.value.indexOf(term) > -1;
            });
            response.apply(null, [filteredValues, "success"]);
          } else {
            console.log('request');
